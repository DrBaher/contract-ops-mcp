...
- **Signing is human-gated and cannot be performed through this server.** Only read/verify ops are exposed (`verify_signature`, `verify_receipt`, `audit_show`). Even the `run`/`catalog` escape hatches enforce this — a `sign` subcommand outside the read-only allowlist (request-create/send/sign/approve, key rotation, TSA anchoring, …) is refused, not executed. To request or apply a signature, use [sign-cli](https://github.com/DrBaher/sign-cli)'s own MCP server, which gates the human gesture behind a per-signer, single-use approval token the agent never holds.
